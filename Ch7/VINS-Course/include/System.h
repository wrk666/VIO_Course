...
//    void PubImageData(double dStampSec, cv::Mat &img);
    void PubImageData(double dStampSec, vector<Matrix<double,6,1>> &img);
    std::recursive_mutex prev_lock_;//避免多线程写导致被改变
...
private:
    ...
    // condition_variable，百度总结：总的来说，std::condition_variable一般用于，一个线程控制另一个线程的运行，
    // 一般是主线程控制子线程运行。 成员函数也比较简单，就notify通知wait_的线程运行就可以了。
