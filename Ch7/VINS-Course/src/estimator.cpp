...
void Estimator::processIMU(double dt, const Vector3d &linear_acceleration, const Vector3d &angular_velocity)
{
    ...
    {
        pre_integrations[frame_count] = new IntegrationBase{acc_0, gyr_0, Bas[frame_count], Bgs[frame_count]};//IMU预积分
    }
    if (frame_count != 0)
    {
        ...
        //中值积分预测系统的状态PQV,从i极分到j
        int j = frame_count;
        Vector3d un_acc_0 = Rs[j] * (acc_0 - Bas[j]) - g;//用前一时刻的Rs，Rs即Qbi_bt
        Vector3d un_gyr = 0.5 * (gyr_0 + angular_velocity) - Bgs[j];//1/2(k + k+1) - bias_gyro
        Rs[j] *= Utility::deltaQ(un_gyr * dt).toRotationMatrix();//wt即为theta，转成四元数更新到现有的R上去
        Vector3d un_acc_1 = Rs[j] * (linear_acceleration - Bas[j]) - g;//用更新之后的Rs
        Vector3d un_acc = 0.5 * (un_acc_0 + un_acc_1);//中值积分
        Ps[j] += dt * Vs[j] + 0.5 * dt * dt * un_acc;//Position vt + 1/2 a * t^2
        Vs[j] += dt * un_acc;//v0+at
    }
    acc_0 = linear_acceleration;
    gyr_0 = angular_velocity;
}
...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, double header)
{
    ...
    //VINS-MONO中关于marg的那个操作，看第二新的帧是否为关键帧
    if (f_manager.addFeatureCheckParallax(frame_count, image, td))//计算特征，check 视差
    ...
    ImageFrame imageframe(image, header);//feature打包，时间戳
    imageframe.pre_integration = tmp_pre_integration;
    all_image_frame.insert(make_pair(header, imageframe));
    tmp_pre_integration = new IntegrationBase{acc_0, gyr_0, Bas[frame_count], Bgs[frame_count]};

    //2表示对系统一无所知，首先要进行外参的标定，2 means don't know anything about extrinsic parameters
    if (ESTIMATE_EXTRINSIC == 2)
    {
        ...
        if (frame_count != 0)
        {
            ...
            if (initial_ex_rotation.CalibrationExRotation(corres, pre_integrations[frame_count]->delta_q, calib_ric))
            {
                ...
                ric[0] = calib_ric;
                RIC[0] = calib_ric;
                ESTIMATE_EXTRINSIC = 1;//标定完成之后置为1，可以对VINS系统进行初始化了
            }
        }
    }
    ...
    if (solver_flag == INITIAL)
    {
        ...
        {
            bool result = false;
            if (ESTIMATE_EXTRINSIC != 2 && (header - initial_timestamp) > 0.1)//判断是否已经初始化
            {
                // cout << "1 initialStructure" << endl;
                result = initialStructure();//初始化系统，该部分可以放到后面来看，第一次看可以重点关注后端的整个求解过程
                initial_timestamp = header;
            }
            if (result)
            {
                solver_flag = NON_LINEAR;
                solveOdometry();//重点求解函数
                slideWindow();
            ...
            frame_count++;//所有数据的frame计数
    }
    else
    {
        TicToc t_solve;
        solveOdometry();//重点函数，看
...
bool Estimator::initialStructure()
{
    ...
    Vector3d relative_T;
    int l;
    if (!relativePose(relative_R, relative_T, l))//这里过不去，导致初始化不成功，跑不起来，可能是没有告诉点的匹配情况,或者是没有找到匹配情况，需要debug
    {
...
bool Estimator::relativePose(Matrix3d &relative_R, Vector3d &relative_T, int &l)
{
    ...
    for (int i = 0; i < WINDOW_SIZE; i++)
    {
        ...
        corres = f_manager.getCorresponding(i, WINDOW_SIZE);//找window里面的第i帧和最后一帧的
        if (corres.size() > 20)
        {
...
void Estimator::double2vector()
{
    ...
    // relative info between two loop frame(这里竟然有回环检测)
...
//marg的一些操作，主要是构建H矩阵，保留之前的info，并把这帧的info继续传下去
void Estimator::MargOldFrame()
{
    ...
    problem.Marginalize(marg_vertex, pose_dim);//执行marg操作
...
//如果第二新非KF（说明可能没怎么运动），则直接marg掉，IMU积分，扔掉视觉观测
...
void Estimator::problemSolve()
{
    ...
    backend::Problem problem(backend::Problem::ProblemType::SLAM_PROBLEM);
    vector<shared_ptr<backend::VertexPose>> vertexCams_vec;
    vector<shared_ptr<backend::VertexSpeedBias>> vertexVB_vec;//速度误差为什么要优化？
    int pose_dim = 0;

    //将各种变量加到problem中：视觉特征点，pose，imu约束，marg的先验约束；然后求解，统计error是否下降，并相应地调整阻尼因子大小（但是这里没有fix第一帧或者给很大的先验的操作，是用来解决H不满秩或者整个traj乱漂的问题）
    // 先把 外参数 节点加入图优化，这个节点在以后一直会被用到，所以我们把他放在第一个（这里仅仅是一帧的pose，难道是第一帧的？）
    shared_ptr<backend::VertexPose> vertexExt(new backend::VertexPose());
    {
        Eigen::VectorXd pose(7);
        pose << para_Ex_Pose[0][0], para_Ex_Pose[0][1], para_Ex_Pose[0][2], para_Ex_Pose[0][3], para_Ex_Pose[0][4], para_Ex_Pose[0][5], para_Ex_Pose[0][6];
    ...
    //这应该是sliding window里面的所有camera的pose节点
    for (int i = 0; i < WINDOW_SIZE + 1; i++)
    {
        shared_ptr<backend::VertexPose> vertexCam(new backend::VertexPose());
        Eigen::VectorXd pose(7);
    ...
    vector<shared_ptr<backend::VertexInverseDepth>> vertexPt_vec;
    {
        ...
        for (auto &it_per_id : f_manager.feature)
        {
            ...
            for (auto &it_per_frame : it_per_id.feature_per_frame)
            {
                ...
                //重投影误差边,这是4元边了：逆深度，先后两次观测的pose，第一个camera pose（为什么非要地一个camera的pose？）
                std::shared_ptr<backend::EdgeReprojection> edge(new backend::EdgeReprojection(pts_i, pts_j));
                std::vector<std::shared_ptr<backend::Vertex>> edge_vertex;
                edge_vertex.push_back(verterxPoint);//逆深度
                edge_vertex.push_back(vertexCams_vec[imu_i]);//第一次观测的位姿Ti
                edge_vertex.push_back(vertexCams_vec[imu_j]);//第二次观测的位姿Tj
                edge_vertex.push_back(vertexExt);//第一个节点？

                edge->SetVertex(edge_vertex);
                edge->SetInformation(project_sqrt_info_.transpose() * project_sqrt_info_);//设置信息矩阵J^T*J
...
void Estimator::backendOptimization()
{
    TicToc t_solver;
    // 借助 vins 框架，维护变量
    vector2double();//solver里面操作指针，所以将vector转为数组
    // 构建求解器
    problemSolve();
    // 优化后的变量处理下自由度
    double2vector();//由于没有fix第一帧或者加入大先验来固定第一帧，所以优化完之后用Twc0将traj拉回来
    //ROS_INFO("whole time for solver: %f", t_solver.toc());

    // 维护 marg
    TicToc t_whole_marginalization;
    if (marginalization_flag == MARGIN_OLD)//如果需要marg掉最老的一帧，则进行相应操作
    {
        vector2double();
