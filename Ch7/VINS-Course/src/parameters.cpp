...
void readParameters(string config_file)
{
    ...
    ACC_N = fsSettings["acc_n"];//ch2 imu 噪声
    ACC_W = fsSettings["acc_w"];
    GYR_N = fsSettings["gyr_n"];
    GYR_W = fsSettings["gyr_w"];

    double times_exp = 10;//10倍噪声
    ACC_N *= times_exp ;
    ACC_W *= times_exp ;
    GYR_N *= times_exp ;
    GYR_W *= times_exp ;
    ACC_N *= sqrt(200) ;   //离散imu噪声
    ACC_W /= sqrt(200) ;
    GYR_N *= sqrt(200) ;
    GYR_W /= sqrt(200) ;

    printf("ACC_N: %f, ACC_W: %f, GYR_N: %f, GYR_W: %f\n", ACC_N, ACC_W, GYR_N, GYR_W);
    ...
    EQUALIZE = fsSettings["equalize"];//是否直方图均衡
