 ...
 *
 * \param p image coordinates
 * \param P coordinates of the projective ray 在投影线上，而不是归一化平面上
 */
void
CataCamera::liftProjective(const Eigen::Vector2d& p, Eigen::Vector3d& P) const
{
    ...
    // Lift points to normalised plane 使用内参的反变换将像素转换到归一化平面，用于去畸变
    mx_d = m_inv_K11 * p(0) + m_inv_K13;
    my_d = m_inv_K22 * p(1) + m_inv_K23;
    ...
    else
    {
        ...
        else
        {
            ...
            int n = 8;
            Eigen::Vector2d d_u;
            distortion(Eigen::Vector2d(mx_d, my_d), d_u);//拟合畸变模型
            // Approximate value
            mx_u = mx_d - d_u(0);
            my_u = my_d - d_u(1);
