...
using Line = std::pair<Eigen::Vector4d, Eigen::Vector4d>;//两点构成一条直线
...
void CreatePointsLines(Points& points, Lines& lines)
{
    ...
    while(!f.eof())
    {
        ...
        if(!s.empty())
        {
            ...
            ss >> y;
            ss >> z;
            Eigen::Vector4d pt0( x, y, z, 1 );//齐次的归一化坐标，而不是在归一化平面上
            ss >> x;
            ss >> y;
            ss >> z;
    ...
    int n = points.size();
    for (int j = 0; j < n; ++j) {
        Eigen::Vector4d p = points[j] + Eigen::Vector4d(0.5,0.5,-0.5,0);//加这个是什么意思？
        points.push_back(p);
    }
...
int main(){
    ...
    save_Pose("imu_pose_noise_10.txt", imudata_noise);//输出文件名称也区分一下
    ...
    for (float t = params.t_start; t<params.t_end;) {
        ...
        //生成IMU数据，包括rotation和translation
    ...
    // points obs in image  在img中观测到的points
    for(int n = 0; n < camdata.size(); ++n)
    {
        ...
        for (int i = 0; i < points.size(); ++i) {
            ...
            Eigen::Vector2d obs(pc1(0)/pc1(2), pc1(1)/pc1(2)) ;//除以深度，成为camera系下的归一化平面上的点，即（x,y,1）,还不在像素平面，还需要使用内参变成像素平面的uv
    ...
    for(int n = 0; n < camdata.size(); ++n)
    {
        ...
        for (int i = 0; i < lines.size(); ++i) {
            ...
            Eigen::Vector4d pc1 = Twc.inverse() * linept.first; // T_wc.inverse() * Pw  -- > point in cam frame 第一个点
            Eigen::Vector4d pc2 = Twc.inverse() * linept.second; // T_wc.inverse() * Pw  -- > point in cam frame 第二个点
