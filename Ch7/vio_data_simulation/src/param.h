...
public:
    ...
    // noise
    double factor = 10;//10倍IMU噪声
    double gyro_bias_sigma = 1.0e-5 * factor;
    double acc_bias_sigma = 0.0001 * factor;

    double gyro_noise_sigma = 0.015 * factor;    // rad/s * 1/sqrt(hz)
    double acc_noise_sigma = 0.019 * factor;      //　m/(s^2) * 1/sqrt(hz)
