...
void IMU::addIMUnoise(MotionData& data)
{
    ...
    std::normal_distribution<double> noise(0.0, 1.0);//这里是数据噪声的参数，增大var可以增大噪声

    Eigen::Vector3d noise_gyro(noise(generator_),noise(generator_),noise(generator_));
    Eigen::Matrix3d gyro_sqrt_cov = param_.gyro_noise_sigma * Eigen::Matrix3d::Identity();//这是噪声的信息矩阵？
