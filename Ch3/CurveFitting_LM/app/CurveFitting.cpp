...
public:
    ...
    CurveFittingVertex(): Vertex(3) {}  // abc: 三个参数， Vertex 是 3 维的，构造时id_自动+1
...
//        residual_(0) = std::exp( abc(0)*x_*x_ + abc(1)*x_ + abc(2) ) - y_;  // 构建残差 y = exp(ax^2+bx+c)
        residual_(0) = ( abc(0)*x_*x_ + abc(1)*x_ + abc(2) ) - y_;  // 构建残差 y = ax^2+bx+c
    }
    ...
    virtual void ComputeJacobians() override
    {
        ...
        Eigen::Matrix<double, 1, 3> jaco_abc;  // 误差为1维，状态量 3 个，所以是 1x3 的雅克比矩阵（严格来说是个3*1的矩阵，jacobian跟分母维度相同）
//        jaco_abc << x_ * x_ * exp_y, x_ * exp_y , 1 * exp_y; //y = exp(ax^2+bx+c) 时的jocabian
        jaco_abc << x_ * x_, x_ , 1; //y = x^2+bx+c 时的jocabian
...
int main()
{
    ...
    for (int i = 0; i < N; ++i) {
        ...
        // 观测 y
//        double y = std::exp( a*x*x + b*x + c ) + n; //exp(ax^2=bx+c)
        double y = ( a*x*x + b*x + c ) + n; //ax^2=bx+c
//        double y = std::exp( a*x*x + b*x + c );

        // 每个观测对应的残差函数
        shared_ptr< CurveFittingEdge > edge(new CurveFittingEdge(x,y));
        std::vector<std::shared_ptr<Vertex>> edge_vertex;
        edge_vertex.push_back(vertex);//顶点的vec推入
        edge->SetVertex(edge_vertex);//设置边连接的顶点
