...
int main() {
    ...
    GetSimDataInWordFrame(cameras, points);//生成仿真数据
    Eigen::Quaterniond qic(1, 0, 0, 0);
    Eigen::Vector3d tic(0, 0, 0);

    // 构建 problem
    Problem problem(Problem::ProblemType::SLAM_PROBLEM);//定义为SLAM problem

    // 所有 Pose
    // 构建pose部分的H
    vector<shared_ptr<VertexPose> > vertexCams_vec;
    for (size_t i = 0; i < cameras.size(); ++i) {
        shared_ptr<VertexPose> vertexCam(new VertexPose());//构造一个顶点
        Eigen::VectorXd pose(7);//7维pose，因为在流型上，所以自由度是6维
        pose << cameras[i].twc, cameras[i].qwc.x(), cameras[i].qwc.y(), cameras[i].qwc.z(), cameras[i].qwc.w();
        vertexCam->SetParameters(pose);//设置待优化顶点的参数

        //将第一个pose fix住，如果不fix，第一个会变，则导致整段轨迹都有漂移（可以先尝试不fix住，看看效果，然后再fix住，进行对比）
//        if(i < 2)
//            vertexCam->SetFixed();

        problem.AddVertex(vertexCam);//将顶点加入problem
        vertexCams_vec.push_back(vertexCam);
    }

    // 所有 Point 及 edge
    std::default_random_engine generator;
    std::normal_distribution<double> noise_pdf(0, 1.);//N(0,1)分布的噪声
    double noise = 0;
    vector<double> noise_invd;
    vector<shared_ptr<VertexInverseDepth> > allPoints;//逆深度Vertex也是继承Vertex类的
    for (size_t i = 0; i < points.size(); ++i) {
        ...
        double inverse_depth = 1. / (Pc.z() + noise);//逆深度
//        double inverse_depth = 1. / Pc.z();
        ...
        for (size_t j = 1; j < cameras.size(); ++j) {
            Eigen::Vector3d pt_i = cameras[0].featurePerId.find(i)->second;//起始帧，此一次
            Eigen::Vector3d pt_j = cameras[j].featurePerId.find(i)->second;//当前帧，measurement
            shared_ptr<EdgeReprojection> edge(new EdgeReprojection(pt_i, pt_j));
            edge->SetTranslationImuFromCamera(qic, tic);//为什么要设置外参Tic？IMU好像没有用到吧

            std::vector<std::shared_ptr<Vertex> > edge_vertex;//这里vertex指的是待优化变量，相当于factor
            //视觉重投影误差，此边为三元边
            edge_vertex.push_back(verterxPoint);//逆深度，camera pose
            edge_vertex.push_back(vertexCams_vec[0]);//第一个camera的pose的Vertex
            edge_vertex.push_back(vertexCams_vec[j]);
            edge->SetVertex(edge_vertex);//设置边所连接的变量

            problem.AddEdge(edge);//添加边，三元边
        }
    }

    //给前两帧camera pose添加增加先验边，gauge prior
    vector<double> Weight{0, 1e1, 1e2, 1e3, 1e4, 1e5, 1e6, 1e7, 1e8, 1e9, 1e10, 1e15};
    double Wp = Weight[11];
    for (size_t k = 0; k < 2; ++k) {
        shared_ptr<EdgeSE3Prior> edge_prior(new EdgeSE3Prior(cameras[k].twc, cameras[k].qwc));//边为什么要传入camera的pose，这里因为是算的prior的Jacobian，要看具体怎么算来决定传入什么参数
        std::vector<std::shared_ptr<Vertex> > edge_prior_vertex;//这里vertex指的是待优化变量，相当于factor
        edge_prior_vertex.push_back(vertexCams_vec[k]);//camera的pose的Vertex，一元边
        edge_prior->SetVertex(edge_prior_vertex);//设置边所连接的变量
        edge_prior->SetInformation(edge_prior->Information() * Wp);//设置权重
        problem.AddEdge(edge_prior);//添加prior边
    }

    problem.Solve(5);//求解时最多会迭代5次
