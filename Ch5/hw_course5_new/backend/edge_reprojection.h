 ...
 * 路标点的逆深度InveseDepth、第一次观测到该路标点的source Camera的位姿T_World_From_Body1，(这里因为所有camera对所有landmark都有观测，所以这里的第一帧都是第一个camera pose，即camera[0])
 * 和观测到该路标点的mearsurement Camera位姿T_World_From_Body2。（不是很理解）
...
private:
    //Translation imu from camera(Tic，camera与IMU外参)
    Qd qic;
    Vec3 tic;
